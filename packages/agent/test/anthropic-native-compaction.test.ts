...
describe("compact() Anthropic native lane", () => {
	...
	test("leads the instructions with the profile identity paragraph without touching the cached prefix", async () => {
		const model = makeAnthropicModel();
		const identity = "The assistant is Fable; the user is Rosalia.";
		const respond = () =>
			assistantMessage(model, {
				providerPayload: { type: "anthropicCompaction", provider: "anthropic", content: NATIVE_SUMMARY },
				stopDetails: { type: "compaction" },
				usage: { ...ZERO_USAGE, input: 64, cacheRead: 79_000, totalTokens: 79_064 },
			});

		const named = recordingCompleteImpl(respond);
		await compact(makePreparation(), model, "sk-ant-test", undefined, undefined, {
			completeImpl: named.completeImpl,
			remoteSystemPrompt: ["You are the live agent."],
			identity,
		});
		const [namedCall] = named.calls;
		const instructions = namedCall.options.anthropicCompaction?.instructions ?? "";
		// This lane writes the durable summary prose itself, so the person
		// register has to reach it — through the instructions, ahead of the
		// structure prompt. The request still replays the live turn verbatim.
		expect(instructions).toContain(`${identity}\n\n`);
		expect(instructions.indexOf(identity)).toBeLessThan(instructions.indexOf("## Goal"));
		expect(namedCall.ctx.systemPrompt).toEqual(["You are the live agent."]);

		const anonymous = recordingCompleteImpl(respond);
		await compact(makePreparation(), model, "sk-ant-test", undefined, undefined, {
			completeImpl: anonymous.completeImpl,
			remoteSystemPrompt: ["You are the live agent."],
		});
		expect(anonymous.calls[0].options.anthropicCompaction?.instructions ?? "").not.toContain(identity);
	});
