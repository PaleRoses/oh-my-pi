...
export async function compact(
	...
	if (
		...
		const basePrompt = summaryOptions.promptOverride ?? SUMMARIZATION_PROMPT;
		const identity = summaryOptions.identity?.trim();
		try {
			const remote = await requestAnthropicNativeCompaction(
				...
				apiKey,
				{
					...
					instructions: buildAnthropicCompactionInstructions(
						// Keep the cached system prefix unchanged; identity belongs in compaction instructions.
						identity ? `${identity}\n\n${basePrompt}` : basePrompt,
						customInstructions,
