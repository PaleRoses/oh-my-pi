...
export interface FooterHost<TSession extends FooterSession = FooterSession> {
	gitEnabled(): boolean;
	/** Effective-identity badge (`principal@bank/project:…`); a display-only session has none. */
	identityBadge?(session: TSession): string | undefined;
}
