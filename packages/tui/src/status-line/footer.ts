 ...
 */
export class FooterComponent<TSession extends FooterSession = FooterSession> implements Component {
	...
	constructor(
		...
		private readonly host: FooterHost<TSession>,
	) {}
	...
	render(width: number): readonly string[] {
		...
		// The badge names the memory principal the session acts for; a
		// display-only session has no effective identity and renders without it.
		const badge = this.host.identityBadge?.(this.session);
		const identityPrefix = badge ? `${badge} • ` : "";

		// Add thinking level hint when the current model advertises supported efforts
		let rightSide = `${identityPrefix}${modelName}`;
		if (state.model?.thinking) {
			if (this.session.isAutoThinking) {
				...
				rightSide = `${identityPrefix}${modelName} • ${resolved ? resolved : `${theme.thinking.autoPending} auto`}`;
			} else {
				const thinkingLevel = state.thinkingLevel ?? ThinkingLevel.Off;
				rightSide = `${identityPrefix}${modelName} • ${thinkingLevel}`;
			}
		}
