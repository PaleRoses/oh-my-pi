...
export class MemoryProtocolHandler implements ProtocolHandler {
	...
	async resolve(url: InternalUrl, context?: ResolveContext): Promise<InternalResource> {
		...
		// artifacts cannot leak across backend changes. A contextless legacy
		// caller inherits the first non-off live backend and is gated with
		// everyone else; only a process with no live memory answers undefined
		// and keeps the registry sweep.
		if (backend !== undefined && backend !== "local") {
