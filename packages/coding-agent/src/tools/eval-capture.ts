...
export interface EvalCaptureResult {
	/**
...
function captureFailure(text: string, reason: string): EvalCaptureResult {
 ...
 */
export async function applyEvalCapture(session: ToolSession, options: EvalCaptureOptions): Promise<EvalCaptureResult> {
	...
	//    schemas and renderers read its bindings during load), and ./eval
	//    reaches the whole tools/index graph, which would cycle at load time.
	let backend: ExecutorBackend;
	try {
		const { resolveSessionPythonBackend } = await import("./eval");
