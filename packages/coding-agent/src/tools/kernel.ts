...
export class EvalTool implements AgentTool<typeof evalSchema> {
	...
	/** Codex Code Mode advertisement, derived from the live session. */
	#codeModeDescription(baseDescription: string): string | undefined {
		...
		const codeMode = resolveCodeMode({
			...
			enabledToolNames,
			kernelTransportAvailable: this.supportsCodeModeTransport(),
		});
	...
	async execute(
		...
		const cellTimeoutMs =
			cells[0].timeoutMs === 0
				? undefined
				: clampTimeout("kernel", cells[0].timeoutMs / 1000, session.settings.get("tools.maxTimeout")) * 1000;
			...
			async ({ jobId, signal: runSignal, reportProgress }) => {
				try {
					...
					if (result.isError === true) {
						...
						throw new ToolError(finalText || "Kernel cell failed");
					}
		...
		if (waitResult.kind === "aborted") {
			...
			throw new ToolAbortError(latestText || "Kernel cell aborted");
		}
