...
export class SessionTools {
	...
	#currentPromptModelKey(): string | undefined {
		...
		// With `includeModelInPrompt: false` the key collapses to the model's
		// delegation bias. The selected profile remains an explicit cache boundary.
		const modelKey = !activeModel
			? undefined
			: this.#host.settings.get("includeModelInPrompt")
	...
	/** True when any memory-backend tool is currently registered. */
	hasMemoryTools(): boolean {
		return MEMORY_BACKEND_TOOL_NAMES.some(name => this.#toolRegistry.has(name));
	}
	...
	async buildSystemPromptForAgentStart(
		...
		if (this.#identity.memory.status !== "enabled") return { systemPrompt: this.#baseSystemPrompt };
	 ...
	 *   4. Sorted names of mounted xd:// skill readers: mounting, unmounting,
	...
	#computeAppliedToolSignature(
		...
		// opportunistically on the next genuine rebuild. The calendar date is not
		// part of the signature either: the date/cwd reminder rides on the first
		// user turn at request time. Direct Code Mode names render the restricted
		// tool inventory, so a codeModeDirectTools change must rebuild even when
		...
		const mountedReaderSegment = mountedTools
			...
			.sort()
			.join("\u0002");
		return `${nameSegment}\u0003${descriptionSegment}\u0007${instructionsSegment}${directSegment}\u0009${mountedReaderSegment}`;
	}
