...
export const hindsightBackend: MemoryBackend = {
	...
	async start(options: MemoryBackendStartOptions): Promise<void> {
		...
		if (options.taskDepth > 0) {
			...
			const parentSlot = parent?.session.getHindsightSessionState();
			const livePrimary = parentSlot?.isAlias ? parentSlot.aliasOf : parentSlot;
			if (!livePrimary) {
			...
			// Aliases don't run auto-recall/auto-retain, but explicit tool calls
			// may have queued writes. Retire that queue before releasing the state.
			await previous?.retireRetainQueue();
			previous?.dispose();
			return;
		}
	...
	async clear(_agentDir, _cwd, session): Promise<void> {
		...
		const state = session?.getHindsightSessionState();
		if (state) await state.retireRetainQueue();
		const previous = session?.setHindsightSessionState(undefined);
		previous?.dispose();
		logger.warn(
	...
	async preCompactionContext(
		...
		const state = session?.getHindsightSessionState();
		if (!state || state.isAlias) return undefined;
...
async function installPrimaryState(
	...
	// before the drain guarantees every previously accepted retain lands on its
	// captured route. Re-read after the await so a concurrent owner cannot leave
	// the actual current state undisposed.
	...
	if (displaced && displaced !== previous) {
		await displaced.retireRetainQueue();
		displaced.dispose();
	}
	previous?.dispose();
