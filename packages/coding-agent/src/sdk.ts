...
export interface CreateAgentSessionOptions {
	...
	/** Whether the template was explicitly supplied or discovered from SYSTEM_TEMPLATE.md. */
...
async function createAgentSessionScoped(options: CreateAgentSessionOptions): Promise<CreateAgentSessionResult> {
	...
	// which rules are bucketed into this session at all. `agentKind` is already
	// resolved above from the explicit option or the structural task metadata;
	// its values are exactly MAIN_AGENT_RULE_NAME / SUB_AGENT_RULE_NAME.
	...
	try {
		...
		const effectivePromptSource =
			options.systemPrompt !== undefined ||
			(options.systemPromptTemplate !== undefined && options.systemPromptTemplateSource !== "discovered") ||
			(options.customSystemPrompt !== undefined && options.customSystemPromptSource !== "discovered")
				...
				: selectedSystemPromptProfile?.prompt !== undefined ||
					...
					: selectedSystemPromptProfile === undefined &&
						  ((options.customSystemPrompt !== undefined && options.customSystemPromptSource === "discovered") ||
								(options.systemPromptTemplate !== undefined &&
									options.systemPromptTemplateSource === "discovered"))
		...
		const selectedSystemPromptTemplate =
			options.systemPromptTemplateSource !== "discovered" || !selectedSystemPromptProfile
				? options.systemPromptTemplate
				: undefined;
		const selectedCustomSystemPrompt =
			selectedSystemPromptTemplate !== undefined
				? undefined
				: options.customSystemPrompt !== undefined && options.customSystemPromptSource !== "discovered"
		...
		const rebuildSystemPrompt = async (
			...
			const defaultPrompt = await buildSystemPromptInternal({
				...
				systemPromptTemplate: selectedSystemPromptTemplate,
