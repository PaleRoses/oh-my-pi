...
import { StatusLineComponent, type StatusLineSegmentOptions } from "@oh-my-pi/pi-tui/status-line";
...
export class InteractiveMode implements InteractiveModeContext {
	...
	async init(options: InteractiveModeInitOptions = {}): Promise<void> {
		...
		const modelName = modelLabel || (this.session.model?.name ?? "Unknown");
		const providerName = modelLabel ? "" : (this.session.model?.provider ?? "Unknown");
	...
	showPromptSettings(): void {
		this.#selectorController.showPromptSettings();
	}
