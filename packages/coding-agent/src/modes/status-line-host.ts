import type { FooterHost, FooterSession, StatusLineHost, StatusLineSession } from "@oh-my-pi/pi-tui/status-line/host";
import { settings } from "../config/settings";
import type { AgentSession } from "../session/agent-session";
import { getSessionCompactionBoundaries } from "../session/context-usage-runtime";
import { formatAgentIdentityBadge, snapshotAgentIdentity } from "../session/identity";
...
/** Only a live session carries an effective identity; a replica or fixture has none. */
type FooterHostSession = FooterSession & Partial<Pick<AgentSession, "effectiveIdentity">>;

/** Application policy and runtime services consumed by the portable status renderer. */
export const statusLineHost: StatusLineHost<StatusLineHostSession> & FooterHost<FooterHostSession> = {
	getSettings: () => ({
	...
	// Only a live session carries an effective identity; a display-only replica
	// or fixture renders the footer without the badge.
	identityBadge: session =>
		session.effectiveIdentity ? formatAgentIdentityBadge(snapshotAgentIdentity(session as AgentSession)) : undefined,
