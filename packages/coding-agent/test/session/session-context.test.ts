...
describe("buildSessionContext provider-native Anthropic compaction", () => {
	it("projects the opaque replay payload and does not duplicate kept turns", () => {
		const entries = [
			{
				type: "message",
				id: "a-old",
			...
			},
			{
				...
				parentId: "a-old",
				timestamp,
				message: { role: "user", content: "kept inside native payload", timestamp: 2 },
			},
			{
				type: "compaction",
				id: "a-compaction",
				...
				preserveData: {
					anthropicRemoteCompaction: { provider: "anthropic", content: "native summary" },
				},
			},
			{
				type: "message",
				id: "a-after",
				parentId: "a-compaction",
				timestamp,
		...
		const summary = compactionSummary(messages);
		expect(summary.providerPayload).toEqual({
			type: "anthropicCompactionHistory",
			provider: "anthropic",
			content: "native summary",
		});
		expect(messages[1]).toMatchObject({ role: "user", content: "after" });

		const transcript = buildSessionContext(entries, undefined, undefined, { transcript: true }).messages;
		expect(
			transcript.some(message => message.role === "user" && message.content === "kept inside native payload"),
		).toBe(true);
		expect(transcript.some(message => message.role === "compactionSummary")).toBe(true);
		expect(transcript.some(message => message.role === "user" && message.content === "after")).toBe(true);
	});
});
