...
import { MemoryProtocolHandler, splitMemoryGlobPattern } from "@oh-my-pi/pi-coding-agent/internal-urls/memory-protocol";
	...
	it("reads memory with a journal-only ToolSession manager", async () => {
		await withMemoryFixture(async ({ cwd, memoryRoot }) => {
			...
			agentRegistry.register({
				...
				kind: "main",
				sessionFile: null,
				session: { sessionManager: manager, ...memorySessionFields("local") } as unknown as AgentSession,
			});
...
describe("MemoryProtocolHandler — mnemopi bridge (issue #4443)", () => {
	...
	it("refuses a contextless memory://root read while mnemopi is the live backend", async () => {
		await withMnemopiSession(async () => {
			const router = InternalUrlRouter.instance();
