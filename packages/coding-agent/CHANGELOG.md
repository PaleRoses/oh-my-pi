...
- Added broker-owned schedules/heartbeats: the hub `schedule` op upserts one-shot (`at`) or repeating (`every`) schedules per owning session/name, persists their timing in `runtimeDir/schedules.json`, and re-arms from the persisted due time. Each fire is a persisted, acknowledged delivery through the existing IRC path with a synthetic `schedule:<name>` sender: it replays across disconnects and broker restarts until the owning live session accepts it, without autonomous session launch. An optional `while` daemon name cancels the schedule instead of firing while that daemon is not live (checked at fire time, at daemon settle, and at recovery); a future-due schedule pins the broker alive across TUI exit, while a fired delivery can replay through a new broker.
- Added an opt-in `capture` parameter to the bash, grep, and read tools: the tool's full output is saved to the session artifact store and bound to the named Python kernel variable via one hidden assignment cell (`<name> = read("artifact://<id>")` through the session's eval backend), and the transcript keeps only a compact stub (`[captured → <name>: <bytes> bytes, <lines> lines, artifact://<id>]` plus a five-line preview). Capture is explicit per call, Python-only in v1, and degrades gracefully — invalid names, artifact failures, or an unavailable kernel return the original output with one warning line instead of failing the tool call.
