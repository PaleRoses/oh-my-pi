...
- **Official Anthropic Messages server compaction**: only direct `anthropic` provider models using the official `anthropic-messages` endpoint are eligible. Compaction sends the `compact-2026-01-12` beta with the 50,000-token trigger and `pause_after_compaction: true`; exactly one nonempty returned compaction block is persisted as same-provider replay history in `preserveData.anthropicRemoteCompaction`. Compatible third-party gateways are excluded. A rejected request or missing/invalid block is a native compaction failure: with `compaction.remoteEndpoint` configured, the remote method can fall through to that generic summarizer/local summary; otherwise maintenance advances to the next `compaction.methodOrder` entry.
...
When a provider-native remote compaction (V2, V1, or Anthropic) succeeds, local LLM summarization is skipped entirely — the durable history lives in the provider replay payload and the stored `summary` is a placeholder lead-in plus the file-operation list.
...
- `compaction.methodOrder` = `["remote", "snapcompact", "handoff", "shake", "soft"]`. `remote` uses provider-native OpenAI-compatible or official Anthropic server compaction when available; unavailable or failed methods advance to the next preference.
