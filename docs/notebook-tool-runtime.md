...
This document describes current `.ipynb` handling in `coding-agent` and its relationship to the eval-backed Python runtime.
...
## Eval-backed execution path (`src/tools/eval.ts` + `src/eval/py/*`)
...
## 5) Streaming/chunk and display handling (eval-backed path)
...
Eval-backed execution rendering expects:
...
2. copy one desired cell source into a `eval` call with `language: "py"`
