...
- [x] (2026-08-03 02:49Z) Passed coding-agent checks, 60 updater validation tests, the binary build, and the active `omp update --check` command.
- [ ] Record the resulting commit and publication receipt.
...
Implementation and behavioral validation are complete. The source launcher now owns its update path explicitly; official package installations retain the existing release updater. Transaction tests prove that check-only and no-op paths preserve `HEAD`, successful updates create and publish a two-parent merge while detached, and merge conflicts, validation failures, or validation worktree drift restore the original clean checkout. Publication remains before final closeout.
